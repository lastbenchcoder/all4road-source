    ...
    public partial class contactdetail : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ContactOwnerid"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    chkIsDefault.Checked = (_ContactOwner.StatusId == Convert.ToInt32(eStatus.Active)) ? true : false;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (_otherStr == null)
            {
                ...
                ContactOwner.StatusId = (chkIsDefault.Checked) ? Convert.ToInt32(eStatus.Active) : Convert.ToInt32(eStatus.InActive);
                ContactOwner.Contact_Query_Submission = (chkSubmitRequest.Checked) ? "Y" : "N";
                ContactOwner.Contact_UpdatedDate = DateTime.Now;
                ContactOwner.AdministratorId = adminId;

                ContactOwner = bContactOwner.Update(ContactOwner);

                if (String.IsNullOrEmpty(ContactOwner.ErrorMessage))
                {
                    Response.Redirect("/administration/pages/contact.aspx?id=200&redirecturl=admin-ContactOwner-rachna-teracotta");
                }
                else
                {
